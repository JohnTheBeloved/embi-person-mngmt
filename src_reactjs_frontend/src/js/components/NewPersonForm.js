...
export default class NewPersonForm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      firstName: '', lastName: '', age: '', favouriteColour: '', hobby: [],
    };
  ...
  handleColourChange(e) {
    this.setState({ colourIsDirty: true });
 	...
 	handleHobbyChange(selectedOption) {
	    this.setState({
      selectedOption,
	      hobby: selectedOption.map(option => option.value),
    });
  }
  ...
  addPerson(e) {
    ...
    this.setState({
      firstName: '', lastName: '', age: '', favouriteColour: '', hobby: [], ageIsDirty: false, numberIsDirty: false, nameIsDirty: false,
    });
  }
  ...
  render() {
    ...
    return (
      ...
      <form className="add-new-form" onSubmit={this.addPerson}>
        ...
        <div className="input-margin input-display">
          <Select isMulti name="hobby" value={selectedOption} onChange={this.handleHobbyChange} options={options} />
          <span className={this.state.showErrors ? 'showError' : 'hide'}>{this.state.numberError}</span>
        </div>
